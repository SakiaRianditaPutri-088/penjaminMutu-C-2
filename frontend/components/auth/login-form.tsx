...
import { registerAccount, verifyLogin } from "@/lib/storage"
...
export default function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message || (language === "id" ? "Login gagal. Silakan coba lagi." : "Login failed. Please try again."),
      )
    } finally {
      setIsLoading(false)
    }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newUser = registerAccount(email, password, name)
          ...
          <form onSubmit={isLogin ? handleLogin : handleRegister} className="space-y-4">
            {error && (
              <div
                className={`p-3 rounded-lg text-sm ${
                  error.includes("berhasil") || error.includes("successful") || error.includes("sent")
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled={isLoading}
                className="dark:bg-slate-900 dark:border-slate-700 focus:ring-2 focus:ring-blue-500"
              />
            </div>
                ...
                <div className="absolute inset-0 flex items-center">
                  <div className="w-full border-t border-slate-300 dark:border-slate-600">
                    <div className="relative flex justify-center text-xs uppercase">
                      <span className="bg-white px-2 dark:bg-slate-900 text-slate-500 dark:text-slate-400">
                        {language === "id" ? "atau" : "or"}
                      </span>
                    </div>
                  </div>
